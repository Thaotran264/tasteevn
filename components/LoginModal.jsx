...
function LoginModal() {
  ...
  const checkOTP = async (OTP) => {
    ...
    if (OTP == data) {
      setStatusInput({ isEmptyPhone: false, showFormRegister: true, showSendOTP: false, showFormOTP: true, isTrueOTP: true, isShowMsgErrOTP: false })
    }
    else {
      setStatusInput({ isEmptyPhone: false, showFormRegister: true, showSendOTP: true, showFormOTP: true, isTrueOTP: false, isShowMsgErrOTP: true })
    }
  }
  ...
  const sendOTP = async ()  => {
    ...
    if(!validatorOBJ.nameMsg && !validatorOBJ.passwordMsg) {
      ...
      setStatusInput({ showFormOTP: false, showSendOTP: true, showFormRegister: true, isEmptyPhone: false, isShowMsgErrOTP: false })
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose} animation={false} centered size="small">
        ...
        <Modal.Body>
          ...
          >
            <h2 className="text-center formTitle mb-3">{textButton.btnSubmit == 'Tiếp tục' ? 'Đăng nhập' : textButton.btnSubmit}</h2>
            <form onSubmit={handleSubmit}>
                ...
                <>
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      aria-describedby="emailHelp"
                    />
                    {validatorForm.nameMsg && <i className="text-center text-danger ">Tên không được để trống</i>}
                  </div>
                  <div className="mb-3">
                    <label htmlFor="exampleInputPassword1" className="form-label  lable-custom">
              ...
              {!statusInput.showSendOTP && <button type="submit" className="btn-custom button-custom-primary-form w-100 mb-2 "> {textButton.btnSubmit == 'Tiếp tục' ? 'Đăng nhập' : textButton.btnSubmit}</button>}
            </form>
